<?php 
    ...
    $sql = "SELECT 
        ...
        WHERE (vrvSoldInventory.stockNum = ?)";
    ...
    if ($stmt->prepare($sql)) {
        ...
        $stmt->bind_result($vin, $year, $make, $model, $model_num, $msrp, $sale, $type, $location, $condition, $int_color, $ext_color, $status, $options, $miles, $uvw, $vin_num, $description, $standards, $floorplan, $gvwr, $len_ft, $len_in, $slides, $sleeping, $chassis, $engine, $fuel, $bl_water, $gr_water, $fr_water,$img_folder,$video_link);
        $stmt->execute();
        $stmt->store_result();
				...
				<div class="row">
					<table>
	                    <tbody>
	                        ...
	                        <?php if ($msrp && $sale > 0) { ?>
	                        <?php  $discount = $msrp - $sale; ?>
	                        <tr>
	                            <td colspan="2">Discount: <?php echo money_format('%.0n', $discount);?></td>
	                        </tr>
	                        <tr>
	                            <td colspan="2" class="sale-price">Sale: <?php echo money_format('%.0n', $sale);?></td>
	                        </tr>
							<?php } ?>
		...
		<div class="content">
			...
			<div class="row">
				<div class="ten columns centered">
					...
					<p><a href="https://vogtrv.com/checkout_form2.php?stock_num=<?php echo $stocknumber; ?>&charge_amount=<?php echo $charge_amount; ?>&charge_reason=Deposit">Click Here to Continue to Vogt RV's Secure Deposit Portal</a></p>
				</div>
			</div>
		</div>
    </div>
