...
$query = "SELECT 
        ...
        LEFT JOIN vrvInvStandards ON vrvInventory.model_num = vrvInvStandards.model_num AND vrvInventory.model = vrvInvStandards.model AND vrvInventory.year = vrvInvStandards.year where vrvInventory.location = 'South' and type <> 'Other' and status <> 'Sold'";
...
$dealerElem->appendChild( $dom->createElement('id', '3650') );
...
$dealerElem->appendChild($dom->createElement('Address'))
              ->appendChild($dom->createCDATASection('5624 Airport Freeway'));
...
$dealerElem->appendChild($dom->createElement('Phone'))
              ->appendChild($dom->createCDATASection('817-831-4222'));
...
$dom->appendChild( $inventory );
   if ( $dom->save('/home/danvog/vogtrv.com/feed/outbound/vogtMotorizedInventory.xml') ) 
	{echo "good to go";}
	else
	{echo "nogo";}
?>