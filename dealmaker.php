	...
	$sql_zip = 'SELECT zip_codes from vrvRegions';
	$zips = $conn->query($sql_zip) or die(mysqli_error($conn));		
    $allowed_zips = array();
    
    while ($row = $zips->fetch_assoc()) {
		$allowed_zips[] = $row['zip_codes'];
    }
	...
	if (isset($_POST['submit'])) {
	    ...
	    $dealmaker_stock = $_GET['stocknum'];
		$source = 'dealmaker inquiry';
	    $to = $_POST['manager_email'];
	    $subject = 'The Dealmaker - Stock Number: ' . $dealmaker_stock;
	    $expected = array('stock_number', 'year', 'make', 'model', 'model_number', 'dealmaker_price', 'first_name', 'last_name', 'email', 'phone', 'zip_code', 'subscribe');
	    $required = array('first_name', 'last_name', 'email', 'zip_code', 'subscribe');
	    ...
	    $headers = "From: The Dealmaker <vogtrv@vogtrv.com>\r\n";
	...
	<div class="dealmaker-response color-secondary-orange-dk-2">
			<div class="row">
			    ...
			    <?php } elseif ($ok)  { 
			    	...
			    	if (!in_array($_POST['zip_code'], $allowed_zips)) {
					
						if ($unit['specials']) {
							$displayed_price = $unit['special_price'];
						} else {
							$displayed_price = $unit['dealmaker'];
						}
						...
						if ($unit['specials']) {
							...
							$displayed_price = $unit['no_hassle'];
						}
					...
					<h3 class="dealmaker-price">Your Price is  <?php echo money_format('%.0n', $displayed_price); ?>  </h3>
					<p>This special is available until  <span class="exp-date"><?php echo date('M d, Y', $date); ?></span>  and is applicable to this stock number <span class="stk-num"><?php echo $stocknumber; ?></span> and availability.  If this unit is unavailable, we may have similar units in stock. </p>
					<p>If you would like to reserve this unit, contact us, or bring this flyer in with you.</p>
						...
						<?php if ($unit['location'] == 'South') { ?>
							 ...
							 <div class="row">
							 	...
							 	<div class="seven columns">
								 	<img  src="img/dealer-map-south.jpg" class="dealermaker-map">
							 	</div>
							 </div>
						...
						<?php } else { ?>
							 ...
							 <div class="row">
							 	...
							 	<div class="seven columns">
								 	<img  src="img/dealer-map-north.jpg" class="dealermaker-map">
							 	</div>
							 </div>
							 
						<?php } ?>
					
					<?php } else { echo "Someone will contact you shortly"; } ?>
		...
		<?php if (!$_POST || !$ok) { ?>	
	   	...
	   	<div id="dealmaker-form">
	        ...
	        <form name="dealmaker" method="POST" action="dealmaker.php?stocknum=<?php echo $stocknumber; ?>">
	        	...
	        	<input type="hidden" name="dealmaker_price" value="<?php echo money_format('%.0n', $unit['no_hassle']); ?>">
	            ...
	            <div class="row">
             		<ul>
						...
						<li class="field text-centered">
	                		<input type="submit" name="submit" class="blue-btn" id="submit_btn" value="Get a Deal" />
						</li>
             		</ul>
	            </div>
	        </form>
